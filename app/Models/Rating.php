...
class Rating extends Model
{
    ...
    */
    protected $type = [
        Partner::TYPE_HOME => 'App\Models\PartnerHome',
        Partner::TYPE_SERVICE => 'App\Models\PartnerService'
    ];
    ...
    public function getAllRatings($id, $type, $sort = 'desc')
    {
        ...
        if ($type == Partner::TYPE_HOME) {
            $rating->where('reviewrateable_type', $this->type[Partner::TYPE_HOME]);
        } else if($type == Partner::TYPE_SERVICE) {
            $rating->where('reviewrateable_type', $this->type[Partner::TYPE_SERVICE]);
        }
        $rating = $rating->orderBy('created_at', $sort)->get();
        return $rating;
    }
    ...
    public function getRecentRatings($id, $type, $limit = 5, $sort = 'desc')
    {
        ...
        $rating = $rating->orderBy('created_at', $sort)->limit($limit)->groupBy('reviewrateable_id')->get();
        return $rating;
    }
     ...
     * @return mixed
     */
    public function averageRatingByType($id, $type)
    {   
        $user_id = User::whereStatus(User::STATUS_ACTIVE)->pluck('id');
        $avarage = $this->selectRaw('AVG(rating) as avarage')->whereIn('author_id', $user_id)->where('approved', true);
        if ($type == Partner::TYPE_HOME) {
            $avarage->where('reviewrateable_type', $this->type[Partner::TYPE_HOME]);
        } else if($type == Partner::TYPE_SERVICE) {
            $avarage->where('reviewrateable_type', $this->type[Partner::TYPE_SERVICE]);
        }
        $avarage = $avarage->where('reviewrateable_id', $id)->groupBy('reviewrateable_id')->get();
        return $avarage;
    }
}
