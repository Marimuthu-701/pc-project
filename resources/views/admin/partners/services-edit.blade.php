...
$featureToDate   = null;
if (isset($partnerServices->featured_from) && !empty($partnerServices->featured_from)) {
    $featureFromDate = date('d-m-Y',strtotime($partnerServices->featured_from));
}
if (isset($partnerServices->featured_to) && !empty($partnerServices->featured_to)) {
    $featureToDate = date('d-m-Y',strtotime($partnerServices->featured_to));
}
@endphp
    ...
    <div class="row">
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 profile_container">
            <div class="card">
                ...
                <form action="{{ route('admin.partners.services.update', $partnerServices->id) }}" method="post" class="partner_details" enctype="multipart/form-data">
                    {{ method_field('PATCH') }}
                    {{ csrf_field() }}
                    <div class="card-body">
                        <div class="row">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                            ...
                                            <select class="form-control {{ $errors->has('services') ? 'is-invalid' : '' }}" id="services" tabindex="1" name="services">
                                                ...
                                                @if(!empty($services))
                                                    @foreach($services as $key => $value)
                                                        <option value="{{ $value->id }}" @if($value->id == $partnerServices->service_id) selected @endif  >{{ $value->name }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{ $errors->has('father_name') ? 'is-invalid' : '' }}" id="father_name" tabindex="3" name="father_name" placeholder="{{ trans('auth.father_name_placeholder') }}" value="{{ $partnerServices->father_name }}">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" id="name" tabindex="2" name="name" placeholder="{{ trans('auth.partner_name_placeholder') }}" value="{{ $partnerServices->name }}">
                                            @if($errors->has('name'))
                                                <div class="invalid-feedback">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{ $errors->has('qualification') ? 'is-invalid' : '' }}" id="qualification" tabindex="4" name="qualification" placeholder="{{ trans('auth.qualification_placeholder') }}" value="{{ $partnerServices->qualification }}">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="row form-group">
                                            <div class="col-md-6">
                                                <label for="year_of_passing">{{ trans('auth.qualification_certificate') }}<span class="required">&nbsp;*</span></label>
                                            ...
                                            <div class="col-md-6">
                                            @if (count($partnerServices->parnterServiceMedia) > 0)
                                                @foreach($partnerServices->parnterServiceMedia as $key => $value)
                                                    <div class="col-md-6">
                                                        @if($value['type'] != "file")
                                                            <img src="{{ asset('storage/'.App\Models\Partner::SERVICE_MEDIA_PATH.$value['source']) }}" width="100" height="100" class="service-image">
                                                        @else
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{ $errors->has('dob') ? 'is-invalid' : '' }}" id="dob" tabindex="6" name="dob"  placeholder="{{ trans('auth.dob_placeholder') }}" value="{{ date('d-m-Y',strtotime($partnerServices->dob))  }}">
                                            @if($errors->has('dob'))
                                                <div class="invalid-feedback">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                            ...
                                            <select class="form-control {{ $errors->has('year_of_passing') ? 'is-invalid' : '' }}" id="year_of_passing" tabindex="7" name="year_of_passing">
                                                ...
                                                @if(!empty($yearofpassing))
                                                    @foreach($yearofpassing as $key => $value)
                                                        <option value="{{ $value }}" @if($value == $partnerServices->year_of_passing) selected @endif  >{{ $value }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{ $errors->has('working_at') ? 'is-invalid' : '' }}" id="working_at" tabindex="9" name="working_at" placeholder="{{ trans('auth.working_at_placeholder') }}" value="{{ $partnerServices->working_at }}">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{ $errors->has('total_experience') ? 'is-invalid' : '' }}" id="total_experience" tabindex="11" name="total_experience" placeholder="{{ trans('auth.total_experience_placeholder') }}" value="{{ $partnerServices->total_experience }}">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{ $errors->has('total_year_of_exp') ? 'is-invalid' : '' }}" id="charges" tabindex="13" name="charges" placeholder="{{ trans('auth.charges_placeholder') }}" value="{{ $partnerServices->charges }}">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control {{ $errors->has('current_address') ? 'is-invalid' : '' }}" id="current_address" placeholder="{{ trans('auth.current_address_placeholder') }}" tabindex="15" name="current_address">{{ $partnerServices->address }}</textarea>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{ $errors->has('college_name') ? 'is-invalid' : '' }}" id="name_of_the_college" tabindex="7" name="college_name" placeholder="{{ trans('auth.college_name_placeholder') }}" value="{{ $partnerServices->college_name }}">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{ $errors->has('specialization_area') ? 'is-invalid' : '' }}" id="specialization_area" tabindex="10" name="specialization_area" placeholder="{{ trans('auth.specialization_area_placeholder') }}" value="{{ $partnerServices->specialization_area }}">
                                            ...
                                            <select class="form-control {{ $errors->has('shift_timings') ? 'is-invalid' : '' }}" id="shift_timings" tabindex="12" name="shift_timings">
                                                ...
                                                @if(!empty($preferredShiftTime))
                                                    @foreach($preferredShiftTime as $key => $value)
                                                        <option value="{{ $key }}" @if($key == $partnerServices->shift_timings) selected @endif  >{{ $value }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control {{ $errors->has('additional_info') ? 'is-invalid' : '' }}" id="additional_info" tabindex="14" name="additional_info" placeholder="{{ trans('auth.additional_info_placeholder') }}">{{ $partnerServices->additional_info }}</textarea> 
                                        ...
                                        <select class="form-control {{ $errors->has('verified') ? 'is-invalid' : '' }}" id="verified" tabindex="8" name="verified">
                                            @if(count($getVerifiedStatus) > 0)
                                                @foreach($getVerifiedStatus as $key => $value)
                                                    <option value="{{ $key }}" @if($key == $partnerServices->verified) selected="selected" @endif>{{ $value }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div>
