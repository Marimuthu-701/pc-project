        ...
        @if(count($equipmentsDetail) > 0)
            @foreach($equipmentsDetail as $key => $value)
                <tr>
                    ...
                    <td>
                        <button type="button" class="btn  medical-view-btn" data-id="{{ $value['id'] }}" ><i class="fas fa-eye"></i></button>
                        <button type="button" class="btn medical-update-btn" data-id="{{ $value['id'] }}" ><i class="fas fa-edit"></i></button>
                        <button type="button" class="btn medical-delete-btn" data-id="{{ $value['id'] }}" id="delete_alert_{{$value['id']}}" ><i class="fas fa-trash-alt"></i></button>
                    </td>
                </tr>
            @endforeach
        @else
